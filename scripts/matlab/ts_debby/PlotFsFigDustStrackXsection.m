function [ ] = PlotFsFigDustStrackXsection()
...
%    { 'TSD_NONSAL_NODUST' 'NSND' }
  ...
  for icase = 1:Ncases
    ...
    InFname = sprintf('XsectionData/strack_dust_mass_lite_%s.h5', Case);
    InVname = '/dust_mass';
    fprintf('Reading %s (%s)\n', InFname, InVname);
    ...
    for it = 1:Nst
      ...
      DUST(icase,it,:,:) = squeeze(h5read(InFname, InVname, [ 1 1 Z1 T1 ], [ Nx 1 Nz 1 ]));
    end
  end
  fprintf('\n');
  ...
  % Plot: 10 panels (5x2)
  OutFile = sprintf('%s/FsFigDustStrackXsection.jpg', Pdir);
  Fig = figure;
