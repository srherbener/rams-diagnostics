function [ ] = GenMoistRamsInfo()
% GenMoistRamsInfo function to generate RAMS TS Debby moist simulation data (prep for GenMoistRamsInit.m)
  ...
  % GenMoistDpFiles() to create the input for the RAMS MAKEVFILE run. Output
  % only the RH values, with RAMS (i,j,k) location, for RAMS to read in and
  ...
  OutFile = sprintf('%s/MoistRamsInfo.h5', Hdir);
  
  fprintf('*********************************************************************\n');
  fprintf('Extracting RH for RAMS TS Debby moist simulation initialization:\n');
  fprintf('\n');
  ...
  for igrid = 1:Ngrids
    ...
    VarFile = sprintf('%s/var-V-2006-08-22-060000-g%d-MOIST.h5', Hdir, igrid);
    PiVar = 'PI';
    RvVar = 'RV';
    ThetaVar = 'THETA';
