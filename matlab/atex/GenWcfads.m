    ...
    for icase = 1:length(Config.Cases)
        ...
        for ifc = 1:length(Wfcases)
            ...
            for it = 1:length(TstartTimes)
              ...
              % Read in data and do time average
              ...
              % create a time average over the interval T1 to T2
              % make each level a PDF of W
              W_CFAD = squeeze(mean(W_HIST(:,:,T1:T2), 3));
              [ Nx Nz ] = size(W_CFAD);
              S = sum(W_CFAD, 1);
              S = repmat(S, [ Nx 1 ]);
              W_CFAD = W_CFAD ./ S;
              ...
              T = 1;  % due to time average, t becomes a dummy variable
