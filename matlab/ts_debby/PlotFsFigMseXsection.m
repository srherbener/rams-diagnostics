...
%    { 'TSD_NONSAL_NODUST' 'NSND' }
  ...
  for icase = 1:Ncases
    ...
    InFname = sprintf('XsectionData/strack_mse_lite_%s.h5', Case);
    InVname = '/moist_static_energy';
    fprintf('Reading %s (%s)\n', InFname, InVname);
    ...
    for it = 1:Nst
      ...
      MSE(icase,it,:,:) = squeeze(h5read(InFname, InVname, [ 1 1 Z1 T1 ], [ Nx 1 Nz 1 ]));
    end
  end
  fprintf('\n');

  % Make difference plots: SD - NSD
  MSE_DIFF = squeeze(MSE(1,:,:,:) - MSE(2,:,:,:));

  % Plot: 10 panels (5x2)
  OutFile = sprintf('%s/FsFigMseXsection.jpg', Pdir);
  Fig = figure;
