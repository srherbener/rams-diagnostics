function [ ] = PlotFsFigMaxWind()
  ...
  Ploc(4) = Ploc(4) - PlocInc;
  set(Paxes, 'Position', Ploc);
  PlotFsFigBgraph(Paxes, PRESAL_FS_IKE, BarColors, 'b', '', 'Factor', BarLabels, 'Difference (%)', Bylim, Fsize, 1, 1, BarLegText, BarLegLoc );

  Paxes = subplot(2, 2, 4);
  Ploc = get(Paxes, 'Position');
  Ploc(4) = Ploc(4) - PlocInc;
  set(Paxes, 'Position', Ploc);
  PlotFsFigBgraph(Paxes, SAL_FS_IKE, BarColors, 'c', '', 'Factor', BarLabels, 'Difference (%)', Bylim, Fsize, 1, 1, BarLegText, BarLegLoc );
