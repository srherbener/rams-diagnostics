function [ ] = ExtractSampleProfile(InFile, InVname, Xselect, Yselect, OutFile, OutVname)
%ExtractSampleProfile extract a time series of sampled vertical profiles
%
%  This routine will read in the dataset given by InFile and InVname and select
%  columns based on the arguments Xselect and Yselect. Then the columns are averaged
%  into a single column and the time series of these averaged columms are output
%  into the dataset OutVname inside OutFile.
%
...
%
  ...
  fprintf('*************************************************************************\n');
  fprintf('Extracting sample profile:\n');
  fprintf('  Reading: %s (%s)\n', InFile, InVname);
  fprintf('    Xselect: %s\n', Xselect);
  fprintf('    Yselect: %s\n', Yselect);
  fprintf('\n');
