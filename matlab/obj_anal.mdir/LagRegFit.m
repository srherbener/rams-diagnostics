function [ RegCoeff, RegYint, CorCoeff  ] = LagRegFit( X, Y, Lag, Start, Size )
% LagRegFit calculate lag correlation and lag regression coefficient and
% lag regression y-intercept on a time series
%
...
elseif ((iystart < 1) || (iyend > length(Y)))
    ...
    [ RegCoeff, RegYint, CorCoeff ] = RegFit(X(ixstart:ixend),Y(iystart:iyend));
end
