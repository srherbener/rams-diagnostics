function [ ] = PlotThetaE(ConfigFile)
% PlotThetaE function to plot theta-e of simulations
%
% This actually plots theta-e at one hour into the simulation since this is the first
...
Ptime = 120; % plot time "steady state"
...
for icase = 1:length(Config.Cases)
  ...
  fprintf('Plotting Theta-e for case: %s\n', Case)
  fprintf('\n');

  % read in the Vt and coordinate data
  % TE data is (x,y,z,t) where x is radius
  Hfile = sprintf('%s/theta_e_%s.h5', AzavgDir, Case);
  fprintf('Reading HDF5 file: %s\n', Hfile);
  TE = squeeze(hdf5read(Hfile, '/theta_e'));
  ...
  Rvals = R(R1:R2);
  Zvals = Z(Z1:Z2);
  Pdata = squeeze(TE(R1:R2,Z1:Z2,TP))'; % note transpose - for plot command
  Pdata (Pdata  == UndefVal) = nan;

  % if this is the control case, grab the Vt at the start of the sim too
  if (strcmp(Case,ControlCase))
    PdataStart = squeeze(TE(R1:R2,Z1:Z2,TS))'; % note transpose - for plot command
    PdataStart (PdataStart  == UndefVal) = nan;
  end

  % plot
  Pfile  = sprintf('%s/te_%s.jpg', PlotDir, Case);
  Ptitle = sprintf('%s: Theta-e (K), T = %d hr', CaseTitle, Ptime);
  ...
  if (strcmp(Case,ControlCase))
    ...
    Pfile  = sprintf('%s/te_START.jpg', PlotDir);
    Ptitle = sprintf('%s: Theta-e (K), T = %d hr', CaseTitle, Stime);
