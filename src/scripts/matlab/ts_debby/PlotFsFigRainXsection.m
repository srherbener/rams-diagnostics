function [ ] = PlotFsFigRainXsection()
  ...
  for icase = 1:Ncases
    ...
    for it = 1:Nst
      ...
      RAIN(icase,it,:,:) = squeeze(h5read(InFname, InVname, [ 1 1 Z1 T1 ], [ Nx 1 Nz 1 ]));
    end
  end
  fprintf('\n');
  ...
  %   F2 = NSD - NSND
  %   F12 = SD - (SND+NSD) + NSND
  F1  = squeeze(RAIN(2,:,:,:) - RAIN(1,:,:,:));
  F2  = squeeze(RAIN(3,:,:,:) - RAIN(1,:,:,:));
  F12 = squeeze(RAIN(4,:,:,:) -(RAIN(2,:,:,:)+RAIN(3,:,:,:)) + RAIN(1,:,:,:));

  % Plot: 10 panels (5x2)
  OutFile = sprintf('%s/FsFigRainXsection.jpg', Pdir);
  Fig = figure;
